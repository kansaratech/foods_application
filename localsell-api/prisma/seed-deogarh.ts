...
type Seed = {
  name: string;
  // Stable override for the DB slug/username/image key when the display
  // `name` changes later — without it a renamed store would be recreated as
  // a duplicate on the next seed run instead of updating the existing row.
  slug?: string;
...
const STORES: Seed[] = [
  {
    name: 'Devshree Deogarh Mahal',
    slug: 'dgh-deogarh-mahal-rasoi',
...
async function main() {
  ...
  const ownerFor = async (slug: string, name: string) =>
    prisma.user.upsert({
      where: { email: `${slug}-owner@localsell.in` },
      update: {},
      create: {
        email: `${slug}-owner@localsell.in`,
        name: `${name} (Owner)`,
  ...
  // a prod seed run before the localsell.in rebrand may have created it under an
  // older email domain (deogarh-diner@padharo.local / @padharo.in) while the
  // phone stays the same.
  const reviewerEmail = 'deogarh-diner@localsell.in';
  ...
  for (let i = 0; i < STORES.length; i++) {
    ...
    if (existing) {
      // Backfill imagery, opening hours and a renamed display name on a
      // re-run without recreating the store (slug/username/image key stay
      // stable across a rename — see the `slug` override on Seed).
    ...
    const restaurant = await prisma.restaurant.create({
      data: {
        ...
        image,
        logo: image,
        // Store-app (merchant) login: `<slug>@store.localsell.in` / `Store@123`
        username: `${slug}@store.localsell.in`,
