    ...
    <main className="w-full overflow-x-hidden bg-white text-slate-950 dark:bg-gray-900 dark:text-white">
      ...
      <section className="bg-white py-8 dark:bg-gray-900 lg:py-14">
        <div className="relative grid w-full items-stretch px-4 md:px-6 lg:grid-cols-[25%_repeat(3,1fr)] lg:px-0">
          <svg
            aria-hidden="true"
            className="pointer-events-none absolute inset-0 hidden h-[128px] w-full text-[#8c1d40] lg:block"
          ...
          <div className="relative z-10 flex min-h-[128px] items-center border-b-2 border-r-2 border-[#8c1d40] px-6 py-7 lg:border-0 lg:px-[5vw]">
            <h2 className="whitespace-nowrap text-3xl font-medium tracking-[-0.04em] text-slate-950 dark:text-white sm:text-4xl">
              More than{" "}
              <span className="font-serif font-normal italic text-[#8c1d40] dark:text-orange-300">delivery.</span>
            </h2>
          </div>
          ...
          {benefits.map((item) => (
            <Link key={item.title} href={item.href} className="group relative z-10 grid min-h-[128px] grid-cols-[48px_1fr_24px] items-center gap-4 border-b border-slate-200 px-5 py-6 transition hover:bg-[#fff7ef]/70 lg:border-b-0 lg:px-7 xl:grid-cols-[52px_1fr_28px] xl:px-10 dark:border-gray-700 dark:hover:bg-gray-800">
              <span aria-hidden="true" className="absolute -top-[6px] left-[28%] hidden h-[14px] w-[14px] rounded-full border-[3px] border-[#8c1d40] bg-white lg:block dark:bg-gray-900" />
              <item.icon aria-hidden="true" className="h-8 w-8 text-[#f5820a]" strokeWidth={1.7} />
              <span>
              ...
              <FiArrowRight aria-hidden="true" className="h-5 w-5 text-slate-900 transition group-hover:translate-x-1 group-hover:text-[#f5820a] dark:text-white" />
            </Link>
          ))}
        </div>
      </section>
