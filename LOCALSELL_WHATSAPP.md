...
### 4a. `localsell_order_invoice`  ·  UTILITY  ·  to the customer  ·  **NOT YET CREATED**

Sent right after `localsell_order_delivered`, carrying the GST tax invoice PDF
(generated by `invoice.service.ts`) as a document header. **Unlike the 8
templates above, this one cannot be created the same simple way** — a
DOCUMENT header requires uploading a sample PDF at template-creation time,
which the WhatsApp Manager UI handles but the plain template-creation API
call doesn't (needs a separate resumable-upload step first). Create it via
**WhatsApp Manager → Account tools → Message templates → Create template**:

- **Name**: `localsell_order_invoice`
- **Category**: Utility
- **Language**: English (US)
- **Header type**: Document — upload any sample invoice PDF when prompted
  (the actual sample doesn't matter, Meta just needs a real PDF to review the
  format against; a screenshot-to-PDF of a test invoice from this build is fine)
- **Body**
  > Hi {{1}}, here's your invoice for order {{2}}. Thank you for ordering with Localsell.
...
- **Footer** (optional): `Generated by Localsell on behalf of the store.`

At send time the code passes the actual invoice PDF as the header's document
`link` (a public HTTPS URL on `/uploads/invoices/...`) — no re-upload to Meta
needed per-send. Once Meta approves this (usually a few hours for Utility),
run `syncWhatsappTemplates` (Admin → WhatsApp) so the DB row's status flips to
APPROVED — the code already calls `sendWhatsAppDocumentTemplate` on every
DELIVERED order; it just silently console-logs (dev fallback) until the
template exists and Cloud API reports it active.
