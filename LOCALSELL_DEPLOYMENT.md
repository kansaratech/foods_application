...
#    Devshree Kitchen, Deogarh Daily Mart, Shrinath Sweets & Breakfast Corner —
#    0 customers, 4 fully-configured stores). Infra secrets on Configuration
#    (Maps / Stripe / SMTP keys) are preserved.
...
`Admin@123`; vendor `<vendor-email>@localsell.in` / `Vendor@123` (e.g.
`hot-pizza-corner-owner@localsell.in`); store app
`<slug>@store.localsell.in` / `Store@123` (e.g.
`hot-pizza-corner@store.localsell.in`); rider `rider1`/`rider2`/`rider3` /
`Rider@123`. **0 customers** in this seed — the marketplace launches empty of
customer accounts, real signups start from zero. The seed also sets
...
# DB: already covered by the existing midnight backup job on this host — no
# change needed. Manual one-off, if you ever need it outside that job:
...
# uploads: now live in a plain host directory (not a Docker volume) — tar it
# directly. Automated nightly via §12.4; manual one-off:
tar czf /var/backups/localsell-uploads-$(date +%F).tar.gz -C /var/localsell/uploads .
```

Both commands are read-only against a live directory/DB and safe to run while the
stack is up. **§12.4 below wires the uploads side into a nightly cron job with
rotation** — run that instead of doing it by hand on an ongoing basis.

> First time deploying the uploads bind-mount change on a server that's already
> running? See §12.3 — existing files sit in the old `localsell_uploads` Docker
> volume and need to be copied into `/var/localsell/uploads` once, or they'll
> look "gone" after the next `up -d --build`.
...
## 12.3 Deploying the uploads bind-mount change

**Why:** uploaded files (food images, store logos, KYC documents) used to live in
a Docker-managed named volume (`localsell_uploads`). That's fine for the running
container, but it's awkward to back up (needs a throwaway `docker run --rm -v ...`
container just to read the files) and it isn't a real path a replica/rsync job can
point at. `docker-compose.yml` now bind-mounts a plain host directory instead —
default `/var/localsell/uploads`, overridable via `UPLOADS_HOST_DIR` in
`deploy/localsell.env` (§6).

**This changes nothing at the application layer** — `UPLOAD_DIR`,
`PUBLIC_UPLOAD_URL`, and the URLs already stored in the database are untouched.
It only changes where the files sit on the host.

### One-time migration (existing server only — skip on a fresh install)

`SERVER-DEPLOY.sh` (regenerated by `scripts/make-deploy-zip.ps1`) now does this
automatically, before `up -d --build`: it creates `UPLOADS_HOST_DIR` (default
`/var/localsell/uploads`) if missing, and — only if that directory is still
empty **and** the old `localsell_uploads` Docker volume exists — copies the old
volume's contents into it. Safe to re-run: once the directory has files, it's a
no-op. Just run the normal redeploy (§7 / §12) and watch for this line:

```
== uploads directory (host bind-mount, see LOCALSELL_DEPLOYMENT.md section 12.3) ==
  /var/localsell/uploads is empty and an old 'localsell_uploads' Docker volume exists - migrating its contents once...
  migrated (N file(s)). Old volume left in place - remove once verified: docker volume rm localsell_uploads
```

If you're deploying by hand instead of via `SERVER-DEPLOY.sh`, or want to check
the counts match before trusting it, run the same steps manually:

```bash
mkdir -p /var/localsell/uploads
docker run --rm \
  -v localsell_uploads:/from \
  -v /var/localsell/uploads:/to \
  busybox sh -c 'cp -a /from/. /to/'

# sanity check: same file count both sides
docker run --rm -v localsell_uploads:/from busybox sh -c 'ls /from | wc -l'
ls /var/localsell/uploads | wc -l
```

Once the new stack is confirmed serving existing images correctly, the old
volume is no longer referenced by `docker-compose.yml` and can be removed to
reclaim disk:

```bash
docker volume rm localsell_uploads   # only after verifying the copy above
```

### Browser check

Open an existing food/store image or a vendor KYC document URL
(`https://api.localsell.in/uploads/<file>`) uploaded before this change —
it must still load after the redeploy.

---

## 12.4 Automated nightly backup (uploads)

The database already has a nightly (midnight) backup job on this host, outside
this repo — no change needed there. What was still missing: the uploads
directory (food images, store logos, KYC documents) — that's what
`deploy/backup.sh` covers.

**What it does** (reads `UPLOADS_HOST_DIR` straight out of `deploy/localsell.env`,
so it can't drift from what the api container actually uses):

1. `tar` the uploads directory → gzip → `/var/backups/localsell/localsell-uploads-<timestamp>.tar.gz`
2. Deletes backups older than 14 days (both the directory and retention window
   are overridable — see below)

The output is written to a `.partial` file and only `mv`'d into place once
complete, so a failed/interrupted run never leaves a corrupt file that looks
valid.
...
Add (nightly at 2:15am, logging to a file you can check later):

```
15 2 * * * cd <project dir> && bash deploy/backup.sh >> /var/log/localsell-backup.log 2>&1
```

Run it once by hand first to confirm it works before trusting the cron line:

```bash
bash deploy/backup.sh
ls -lh /var/backups/localsell
```

**Overrides** (export before running, or add to the crontab line):
`LOCALSELL_BACKUP_DIR` (default `/var/backups/localsell`),
`LOCALSELL_BACKUP_RETENTION_DAYS` (default `14`).

> A backup nobody has restored isn't verified. Do the restore drill below at
> least once — ideally on a spare box, not this server.

### Restoring uploads

Safe to extract into the live directory — filenames are random UUIDs, so this
only ever adds files back, never overwrites current ones:

```bash
tar xzf /var/backups/localsell/localsell-uploads-<timestamp>.tar.gz -C /var/localsell/uploads
```

### Restoring the database

Not produced by this script — pulled from whichever backup the existing
midnight DB job writes. The manual dump/restore pattern (⚠️ overwrites the live
DB — only against a fresh/target DB, and only intentionally), for reference:

```bash
DBPW=$(cat /root/localsell_db_pw.txt)
gunzip < <path-to-that-job's-dump>.sql.gz \
  | docker exec -i mysql_dev_3308 sh -c 'exec mysql -ulocalsell -p"$0" localsell' "$DBPW"
```
