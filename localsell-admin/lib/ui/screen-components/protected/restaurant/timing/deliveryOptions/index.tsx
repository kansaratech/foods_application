...
const ToggleSchema = Yup.object().shape({
  ...
  deliveryProvider: Yup.string().oneOf(['SELF']).required(),
});
...
const PROVIDER_OPTIONS: {
  value: IDeliveryOptions['deliveryProvider'];
  label: string;
}[] = [
  ...
  const handleSubmit = (values: IDeliveryOptions) => {
    ...
    if (!values.pickup && !values.delivery) {
      return showToast({
        ...
        title: 'Warning',
        message:
          'At least one service option (Pickup or Delivery) must be enabled.',
        duration: 3000,
      });
    }
                    ...
                    {PROVIDER_OPTIONS.map((opt) => (
                      ...
                      >
                        <input
                          ...
                          onChange={() =>
                            setFieldValue('deliveryProvider', opt.value)
                          }
                        />
                        {opt.label}
                      </label>
