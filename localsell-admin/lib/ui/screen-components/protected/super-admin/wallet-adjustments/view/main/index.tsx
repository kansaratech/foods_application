...
export default function WalletAdjustmentsMain() {
  ...
  const { data, loading, refetch } = useQuery<IWalletAdjustmentsResponse>(
    GET_WALLET_ADJUSTMENTS,
    {
      variables: { limit: 50 },
  ...
  ).map((r) => ({ _id: r._id, name: r.name || r.username || r._id }));
  ...
  const submit = async () => {
    ...
    if (!subjectId || Number.isNaN(parsed) || parsed === 0) {
      showToast({
        type: 'error',
        title: t('Error'),
        message: t('Pick a payee and a non-zero amount'),
        duration: 2500,
      });
      return;
    }
      ...
      <div className="mb-4 flex flex-wrap items-end gap-3 rounded border p-3 text-sm dark:border-dark-600">
        <label className="flex flex-col">
          ...
          <Select
            value={subjectType}
            onChange={(e) => {
              setSubjectType(e.value as 'STORE' | 'RIDER');
              setSubjectId('');
            }}
        ...
        <label className="flex flex-col">
          <span className="mb-1 text-gray-500">
            {subjectType === 'STORE' ? t('Store') : t('Rider')}
          </span>
          <Select
            value={subjectId}
            onChange={(e) => setSubjectId(e.value)}
            className="h-10 w-52 rounded border border-gray-300 px-2 dark:bg-dark-950"
          >
        ...
        <label className="flex flex-col">
          ...
          <input
            ...
            placeholder="-250"
            className="ls-field h-10 w-28 rounded border border-gray-300 px-2 dark:bg-dark-950"
          />
        </label>
        <label className="flex flex-col">
          ...
          <Select
            ...
            className="h-10 w-36 rounded border border-gray-300 px-2 capitalize dark:bg-dark-950"
          >
            {REASONS.map((r) => (
        ...
        columns={[
          {
            ...
            body: (r: IWalletAdjustmentRow) => day(r.createdAt),
          },
          {
            headerName: t('Payee'),
            propertyName: 'subjectName',
            body: (r: IWalletAdjustmentRow) =>
              `${r.subjectName ?? r.subjectId} (${r.subjectType})`,
          },
          {
            headerName: t('Amount'),
            propertyName: 'amount',
            body: (r: IWalletAdjustmentRow) => (
              <span
                className={`font-semibold ${r.amount < 0 ? 'text-red-600' : 'text-green-700'}`}
              >
                {r.amount >= 0 ? '+' : ''}
                {money(r.amount)}
              </span>
