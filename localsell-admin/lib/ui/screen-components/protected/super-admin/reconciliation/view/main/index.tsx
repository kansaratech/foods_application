...
export default function ReconciliationMain() {
  ...
  const { data, loading } = useQuery<IReconciliationReportResponse>(
  ...
  const negativeWallets =
    (r?.negativeWalletStores ?? 0) + (r?.negativeWalletRiders ?? 0);
    ...
    <div className="p-4 md:p-6">
      <div className="mb-4 flex flex-wrap items-end gap-3 rounded-xl border border-slate-200 bg-white p-3 dark:border-dark-600 dark:bg-dark-900">
        ...
        <p className="ml-auto max-w-xs text-xs text-slate-400">
          {t(
            'The date range only affects the balance checks below — wallet balances are always shown as of right now.'
          )}
        </p>
      </div>
        ...
        <div className="flex flex-col gap-5">
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
              <div className="rounded-xl border border-slate-200 bg-white p-4 dark:border-dark-600 dark:bg-dark-900">
                <p className="text-xs text-slate-500">
                  {t('Store wallets owed')}
                </p>
                <p className="mt-1 text-xl font-bold text-slate-900 dark:text-white">
                  {money(r.storeWalletOutstanding)}
                </p>
                <p className="mt-1 text-xs text-slate-400">
                  {t('What the platform currently owes stores')}
                </p>
              </div>
              <div className="rounded-xl border border-slate-200 bg-white p-4 dark:border-dark-600 dark:bg-dark-900">
                <p className="text-xs text-slate-500">
                  {t('Rider wallets owed')}
                </p>
                <p className="mt-1 text-xl font-bold text-slate-900 dark:text-white">
                  {money(r.riderWalletOutstanding)}
                </p>
                <p className="mt-1 text-xs text-slate-400">
                  {t('What the platform currently owes riders')}
                </p>
              </div>
              <div
                className={`rounded-xl border p-4 ${negativeWallets > 0 ? 'border-red-200 bg-red-50 dark:border-red-900 dark:bg-red-950/30' : 'border-slate-200 bg-white dark:border-dark-600 dark:bg-dark-900'}`}
              >
                <p className="text-xs text-slate-500">
                  {t('Wallets in the negative')}
                </p>
                <p
                  className={`mt-1 text-xl font-bold ${negativeWallets > 0 ? 'text-red-600' : 'text-slate-900 dark:text-white'}`}
                >
                  {negativeWallets}
                </p>
                <p className="mt-1 text-xs text-slate-400">
                  {r.negativeWalletStores} {t('stores')},{' '}
                  {r.negativeWalletRiders} {t('riders')}
                </p>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              <div className="rounded-xl border border-slate-200 bg-white p-4 dark:border-dark-600 dark:bg-dark-900">
                <p className="text-xs text-slate-500">
                  {t('Rider cash deposits waiting for admin confirmation')}
                </p>
                <p className="mt-1 text-xl font-bold text-slate-900 dark:text-white">
                  {r.pendingRiderDeposits}{' '}
                  {r.pendingRiderDeposits === 1 ? t('deposit') : t('deposits')}
                  <span className="ml-2 text-base font-semibold text-slate-500">
                    {money(r.pendingRiderDepositTotal)}
                  </span>
                </p>
              </div>
            </div>
          ...
          <div>
            ...
            <p className="mb-2 text-xs text-slate-500">
              {t(
                'Each row should balance to ₹0 — a mismatch means money is unaccounted for.'
              )}
            </p>
              ...
              <table className="ls-native-table w-full border-collapse text-sm">
                ...
                <tbody>
                  {r.lines.map((l) => (
                    <tr
                      key={l.label}
                      className="border-b border-slate-100 last:border-0 dark:border-dark-800"
                    >
                      ...
                      <td
                        className={`px-4 py-3 text-right ${Math.abs(l.delta) >= 1 ? 'font-semibold text-red-600' : 'text-slate-400'}`}
                      >
                        {money(l.delta)}
                      </td>
