...
export default function RiderCashMain() {
  ...
  const { data, loading, refetch } = useQuery<IRiderCashOutstandingResponse>(
    GET_RIDER_CASH_OUTSTANDING,
    {
  ...
  const { data: sumData, refetch: refetchSummary } =
    useQuery<IRiderCashSummaryResponse>(GET_RIDER_CASH_SUMMARY, {
  ...
  const [recordRemittance, { loading: recording }] = useMutation(
    RECORD_RIDER_CASH_REMITTANCE
  );
  const [confirmDeposit, { loading: confirming }] = useMutation(
    CONFIRM_RIDER_CASH_DEPOSIT
  );
  ...
  return (
    <div className="p-3">
      ...
      <Table
        ...
        handleRowClick={(e) =>
          setOpenRiderId(
            (e.data as IRiderCashOutstandingRow)?.rider?._id ?? null
          )
        }
        columns={[
          {
            ...
            body: (r: IRiderCashOutstandingRow) =>
              r.rider.name || r.rider.username || '—',
          },
          {
            headerName: t('Unremitted deliveries'),
            propertyName: 'entryCount',
          },
          {
            headerName: t('Oldest'),
          ...
          },
          {
            ...
            body: (r: IRiderCashOutstandingRow) => (
              <span className="font-semibold">{money(r.outstanding)}</span>
            ),
          },
          {
      ...
      <Dialog
        header={
          summary
            ? summary.rider.name || summary.rider.username || t('Rider cash')
            : t('Rider cash')
        }
        visible={!!openRiderId}
        ...
        {summary && (
          <div className="flex flex-col gap-4 text-sm">
            <div className="flex flex-wrap gap-x-8 gap-y-1">
              <span>
                <b>{t('Outstanding')}:</b> {money(summary.outstanding)}
              </span>
              <span>
                <b>{t('Cash limit')}:</b> {money(summary.cashLimit)}
              </span>
              <span>
                <b>{t('Wallet balance')}:</b> {money(summary.walletBalance)}
              </span>
              <span>
                <b>{t('Available to withdraw')}:</b>{' '}
                {money(summary.availableToWithdraw)}
              </span>
              <span>
                <b>{t('Lifetime remitted')}:</b>{' '}
                {money(summary.lifetimeRemitted)}
              </span>
            </div>
            ...
            {summary.remittances.some((r) => r.status === 'PENDING') && (
              <div className="rounded border border-amber-300 bg-amber-50 p-3 dark:bg-dark-900">
                <p className="mb-2 font-semibold">
                  {t('Deposits awaiting confirmation')}
                </p>
              ...
              <div className="flex flex-wrap items-end gap-3 rounded bg-gray-50 p-3 dark:bg-dark-900">
                <label className="flex flex-col">
                  ...
                  <input
                    ...
                    className="ls-field h-10 w-40 rounded border border-gray-300 px-2 dark:bg-dark-950"
                  />
                </label>
                <label className="flex flex-col">
