...
export default function VendorMain({ activeTab }: IVendorMainComponentProps) {
  ...
  useEffect(
    () => setStorePage(1),
    [restaurantContextData.globalFilter, restaurantByOwnerResponse?.data]
  );
  ...
  return (
    <div className="flex min-h-0 flex-grow flex-col gap-4 overflow-hidden bg-slate-50 p-3 sm:flex-row dark:bg-dark-950">
      ...
      >
        ...
        <div className="border-b pb-3 pt-3 dark:border-dark-600">
          ...
          <div className="mb-4 flex items-center justify-between">
            ...
            <TextIconClickable
              ...
              title={t('Add Store')}
              onClick={() =>
                router.push(`/general/stores/create?vendorId=${vendorId ?? ''}`)
              }
            />
          </div>
        ...
        {storeTotal > 0 && (
          <div className="flex shrink-0 items-center justify-between border-t border-slate-200 bg-white px-2 py-3 text-xs dark:border-dark-600 dark:bg-dark-950">
            <span className="text-slate-500">
              Showing {(storePage - 1) * storePageSize + 1}–
              {Math.min(storePage * storePageSize, storeTotal)} of {storeTotal}{' '}
              stores
            </span>
            <div className="flex items-center gap-2">
              <ActionButton
                ...
                className="rounded border px-3 py-1.5 disabled:cursor-not-allowed disabled:opacity-30"
              >
                Previous
              </ActionButton>
