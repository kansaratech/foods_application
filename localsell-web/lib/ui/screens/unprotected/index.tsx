    ...
    <main className="w-full overflow-x-hidden bg-white text-slate-950 dark:bg-gray-900 dark:text-white">
      ...
      <section className="bg-white py-8 dark:bg-gray-900 lg:py-14">
        <div className="relative grid w-full items-stretch px-4 md:px-6 lg:grid-cols-[25%_repeat(3,1fr)] lg:px-0">
          <div className="relative flex min-h-[128px] items-center border-b-2 border-r-2 border-[#16293f] px-6 py-7 lg:border-0 lg:px-[5vw]">
          ...
          {/* Decorative connector rail — drawn last so it stays crisp over the
              card hover fill instead of smearing through a translucent one. */}
          <svg
            aria-hidden="true"
            className="pointer-events-none absolute inset-0 z-10 hidden h-[128px] w-full text-[#16293f] dark:text-gray-600 lg:block"
          ...
          {[32, 57, 82].map((leftPct) => (
            <span
              key={leftPct}
              aria-hidden="true"
              style={{ left: `${leftPct}%` }}
              className="pointer-events-none absolute top-[-6px] z-10 hidden h-[13px] w-[13px] -translate-x-1/2 rounded-full border-[3px] border-[#16293f] bg-white lg:block dark:border-gray-500 dark:bg-gray-900"
            />
          ))}
        </div>
      </section>
