  ...
  return (
    <main className="overflow-hidden bg-white text-slate-950 dark:bg-gray-900 dark:text-white">
      <section className="relative border-b border-slate-200 bg-[#fffaf5] dark:border-gray-800 dark:bg-gray-950">
        <div aria-hidden="true" className="absolute -left-32 top-20 h-80 w-80 rounded-full bg-[#1c5bc7]/10 blur-3xl" />
        <div className="relative grid min-h-[650px] items-center gap-10 px-4 py-12 md:px-6 lg:grid-cols-[0.88fr_1.12fr] lg:px-12 lg:py-16 xl:px-20 2xl:px-[80px]">
          <div className="relative z-10 max-w-2xl">
            <p className="text-[11px] font-bold uppercase tracking-[0.26em] text-[#1c5bc7]">{eyebrow}</p>
            <h1 className="mt-6 text-[42px] font-black leading-[0.98] tracking-[-0.05em] text-slate-950 dark:text-white sm:text-6xl xl:text-[72px]">
              {heading}
              <span className="mt-2 block font-serif text-[38px] font-normal italic leading-[1.04] tracking-[-0.035em] text-[#16293f] dark:text-blue-300 sm:text-[58px] xl:text-[66px]">{accent}</span>
            </h1>
            <p className="mt-7 max-w-xl text-base leading-7 text-slate-600 dark:text-gray-300 sm:text-lg">{intro}</p>
            <div className="mt-9 flex flex-wrap items-center gap-3">
              <a href="#apply" className="inline-flex items-center gap-2 rounded-full bg-[#1c5bc7] px-6 py-3.5 text-sm font-bold text-white shadow-[0_14px_30px_rgba(28,91,199,0.24)] transition hover:-translate-y-0.5 hover:bg-[#1a52b4]">
                {primaryCta} <FiArrowRight aria-hidden="true" />
              </a>
              <a href="#how" className="inline-flex items-center gap-2 rounded-full border border-[#16293f]/30 bg-white px-6 py-3.5 text-sm font-bold text-[#16293f] transition hover:border-[#16293f] dark:bg-gray-900 dark:text-blue-300">
      ...
      <section className="grid border-b border-slate-200 bg-white px-4 py-8 sm:grid-cols-3 md:px-6 lg:px-12 xl:px-20 2xl:px-[80px] dark:border-gray-800 dark:bg-gray-900">
        {metrics.map((metric, index) => (
          <div key={metric.label} className={`px-4 py-3 text-center ${index > 0 ? "sm:border-l sm:border-slate-200 dark:sm:border-gray-700" : ""}`}>
            <strong className="block text-2xl font-black tracking-[-0.03em] text-[#16293f] dark:text-blue-300">{metric.value}</strong>
      ...
      <section className="px-4 py-16 md:px-6 lg:px-12 lg:py-24 xl:px-20 2xl:px-[80px]">
        <div className="grid gap-12 lg:grid-cols-[0.72fr_1.28fr] lg:gap-20">
          <div className="lg:sticky lg:top-24 lg:self-start">
            <p className="text-[11px] font-bold uppercase tracking-[0.24em] text-[#1c5bc7]">{benefitsEyebrow}</p>
          ...
          <div className="grid gap-4">
            {benefits.map((benefit, index) => (
              <article key={benefit.heading} className="group grid gap-5 rounded-[1.75rem] border border-slate-200 bg-white p-6 transition hover:-translate-y-1 hover:border-[#1c5bc7]/45 hover:shadow-[0_22px_55px_rgba(22,41,63,0.08)] sm:grid-cols-[64px_1fr_36px] sm:items-center sm:p-8 dark:border-gray-700 dark:bg-gray-800">
                <span className="grid h-14 w-14 place-items-center rounded-2xl bg-[#e8f0fc] text-xl text-[#1c5bc7] dark:bg-gray-700">{benefit.icon}</span>
      ...
      <section id="how" className="scroll-mt-20 bg-[#0f1d30] px-4 py-16 text-white md:px-6 lg:px-12 lg:py-24 xl:px-20 2xl:px-[80px]">
        <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
          <div><p className="text-[11px] font-bold uppercase tracking-[0.24em] text-[#8fbdf0]">Simple onboarding</p><h2 className="mt-4 max-w-2xl text-4xl font-black tracking-[-0.04em] sm:text-5xl">{stepsHeading}</h2></div>
        ...
        <ol className="mt-12 grid gap-8 md:grid-cols-3 md:gap-0">
          {steps.map((step, index) => (
            <li key={step.title} className="relative border-t border-white/15 pt-8 md:pr-10">
              <span className="absolute -top-3 left-0 grid h-6 w-6 place-items-center rounded-full border-2 border-[#1c5bc7] bg-[#0f1d30] text-[9px] font-bold text-[#8fbdf0]">{index + 1}</span>
