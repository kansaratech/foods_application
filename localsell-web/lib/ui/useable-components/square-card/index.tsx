  ...
  if (cuisines || showLogo) {
    return (
      <button
        ...
        className="group mx-auto mb-5 flex w-[calc(100%-12px)] flex-col items-center gap-2 text-center"
      >
        ...
        <span className="line-clamp-2 min-h-10 w-full text-[13px] font-semibold tracking-[-0.01em] text-slate-800 dark:text-gray-200">
          {item?.name}
        </span>
      </button>
    );
  ...
  if (shoptype) {
    return (
      <button
        type="button"
        onClick={onClickHandler}
        className="group relative mx-auto mb-5 block aspect-[4/3] w-[calc(100%-16px)] max-w-[240px] overflow-hidden rounded-2xl border border-slate-200 shadow-[0_10px_30px_rgba(22,41,63,0.06)] transition duration-300 hover:-translate-y-1 hover:border-[#1c5bc7]/40 hover:shadow-[0_18px_45px_rgba(22,41,63,0.14)] dark:border-gray-700"
      >
        <Image
          src={getImgSrc || fallback}
          alt={item?.name}
